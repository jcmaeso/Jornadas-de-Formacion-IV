...
static void BTN_interrupt_handler(void* GPIO_PIN);
...
typedef struct simonLL_{
	uint8_t* flags;
	int* user_sequence;
	int* user_sequence_position;
	GPIO_TypeDef* GPIO_bank_list[N_LEDS];
	TIM_HandleTypeDef* htim;
	uint16_t GPIO_pin_list[N_LEDS];
	DISPLAY_SHOW_fcn_t show_fcns[10];
...
IRQ_fcn_t* simon_LL_init(void* _htim,uint8_t* _flags, int* _user_sequence, int* _user_sequence_position){
	...
	simonLL->irq_fcns[0] = BTN_interrupt_handler;
	simonLL->irq_fcns[1] = TIM1_interrupt_handler;
